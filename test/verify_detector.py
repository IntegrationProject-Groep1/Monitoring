"""Verifies the detector publishes a HEARTBEAT_CRITICAL alert to `to_mailing` and generates daily reports.
...
def main() -> None:
    ...
    while time.time() < deadline:
        method, _props, body = channel.basic_get(queue="to_mailing", auto_ack=True)
    ...
    if not alert_found:
        connection.close()
        print(f"FAIL: no alert on 'to_mailing' within {TIMEOUT_SECONDS}s", file=sys.stderr)
        sys.exit(1)
